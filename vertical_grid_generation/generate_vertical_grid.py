...
parser = argparse.ArgumentParser(description='Generate a vertical grid for ocean models.')
parser.add_argument('--H', type=float, required=True, help='Maximum ocean depth (meters)')
parser.add_argument('--dzd', type=float, required=True, help='Maximum grid spacing at depth (meters)')
parser.add_argument('--min_dz', type=float, required=True, help='Minimum grid spacing at the surface (meters)')
parser.add_argument('--depfac', type=float, required=True, help='Tuning parameter for grid sharpness')
parser.add_argument('--output', type=str, default='ocean_vgrid.nc', help='Output NetCDF filename')
...
print(f"SUCCESS!! You now have a vertical grid of {len(real_prop_z) - 1} levels with grid spacing ranging from {real_delta_z[0]} to {real_delta_z[-1]} written to file {output_filename}")

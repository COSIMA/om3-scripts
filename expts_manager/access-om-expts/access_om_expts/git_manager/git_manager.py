"""
Payu utilities for the experiment management
"""
...
def check_and_commit_changes(path: str) -> None:
    ...
    If no changes are detected, it prints a message indicating that no commit was made.
    """
    repo = git.Repo(path)
    ...
    commit_message = (
        f"Control experiment setup: Configure `{current_branch}` branch,\n"
        f"committed files/directories {staged_files}!"
    )
...
def _get_deleted_files(repo: git.Repo) -> list[str] | None:
    """
...
def _get_changed_files(repo: git.Repo) -> list[str] | None:
    """
...
def _get_untracked_files(repo: git.Repo) -> list[str] | None:
    """
